...
import TaskGroupServices from '@/services/taskGroup-api'
import { DataTable } from '@/components/Table'
import AddTaskGroup from '@/components/TaskTag/AddTaskGroup'
import { errorMessage, successMessage } from '@/components/ToasterMessage'
import { Button } from '@/components/ui/button'
import { Plus } from 'lucide-react'
import { useEffect, useState } from 'react'
import { useForm } from 'react-hook-form'
import { TaskGroupColumn } from './task-group-column'
...
const TaskGroup = () => {
  ...
  const fetchGroupList = async () => {
    try {
      const response = await TaskGroupServices.GetAllTaskGroup(page, length)
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res = await TaskGroupServices.deleteGroupById(deleteIndex)
    ...
    <>
      <div>
        <div className='mb-3 flex items-center justify-between'>
          <LayoutHeader pageTitle='Task Group' />
        ...
        <DataTable
          columns={TaskGroupColumn(handleDeleteScope, handleEditScope)}
          data={getList}
        ...
        <LeadSettingModal
          ...
          message={editData ? 'Edit Task Group' : 'Add Task Group'}
        />
      </div>
    </>
  )
