...
import AddLeadsType from '@/components/LeadSetting/LeadType/AddLeadType'
import EditLeadType from '@/components/LeadSetting/LeadType/EditLeadType'
...
const LeadsType = () => {
  ...
  const getListLeads = async () => {
    try {
      setLoading(true)
      const res = await LeadsSettingServices.getLeadType(page, length)
      if (res?.status === 200) {
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res = await LeadsSettingServices.deleteLeadTypesById(deleteIndex)
  ...
  return (
    <>
      <div className='mb-3 flex items-center justify-between'>
        <LayoutHeader pageTitle='Lead Type List' />
      ...
      <LeadSettingModal
        ...
        message={editData ? 'Edit Leads Type' : 'Add Leads Type'}
      />
    </>
  )
}
