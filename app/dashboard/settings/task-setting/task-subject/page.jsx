...
import TaskSubjectServices from '@/services/taskSubject-api'
import { DataTable } from '@/components/Table'
import AddSubject from '@/components/TaskTag/AddSubject'
import { errorMessage, successMessage } from '@/components/ToasterMessage'
import { Button } from '@/components/ui/button'
import { Plus } from 'lucide-react'
import { useEffect, useState } from 'react'
import { useForm } from 'react-hook-form'
import { TaskSubjectColumn } from './task-subject-column'
...
const TaskSubject = () => {
  ...
  const fetchSubjectList = async () => {
    try {
      const response = await TaskSubjectServices.GetAllTaskSubject(page, length)
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res = await TaskSubjectServices.deleteSubjectById(deleteIndex)
    ...
    <>
      <div>
        <div className='mb-3 flex items-center justify-between'>
          <LayoutHeader pageTitle='Task Subject' />
        ...
        <DataTable
          columns={TaskSubjectColumn(handleDeleteScope, handleEditScope)}
          data={getList}
        ...
        <LeadSettingModal
          ...
          message={editData ? 'Edit Task Subject' : 'Add Task Subject'}
        />
      </div>
    </>
  )
