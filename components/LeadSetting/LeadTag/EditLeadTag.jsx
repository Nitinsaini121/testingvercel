...
const EditLeadTag = ({editData,getListLeads,handleModalClose}) => {
  ...
  const handleLeadTagSubmit = async data => {
    ...
    try {
      ...
      // Submit to API
      const response = await LeadsSettingServices.UpdateTagById(
        id,
        formData
      )
  ...
  return (
    <>
      <FormProvider {...form}>
        <form
          onSubmit={form.handleSubmit(handleLeadTagSubmit)}
