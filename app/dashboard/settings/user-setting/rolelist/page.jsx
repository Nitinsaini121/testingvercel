...
import { RolesColumn } from './role-column'
import LeadSettingModal from '@/components/LeadsettingModal'
import EditUserRole from '@/components/UserSetting/EditUserRole'
...
const UserList = () => {
  ...
  const fetchUser = async () => {
    try {
      const response = await UserServices.GetAllUserRoles(page, length)
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res = await UserServices.deleteUserRoleById(deleteIndex)
  ...
  return (
    <>
      <div className='mb-3 flex items-center justify-between'>
        <LayoutHeader pageTitle='All Role' />
        <Button
          className='site-button'
          onClick={() => router.push(`/dashboard/settings/user-setting/addrole`)}
        >
          <Plus />
          Add Role
        </Button>
      ...
      <DataTable
        data={user}
        loading={loading}
        columns={RolesColumn(handleDeleteLeads, handleEditLeads)}
